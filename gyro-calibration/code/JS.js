...
function loadselect1(){
    ...
    const keyFiles = document.getElementById('key');
    ...
    while (baseFoldersSelect2.options.length > 1) { // Remet a zéro pour éviter les problèmes (select 3 & 3 pop up)
        baseFoldersSelect2.remove(baseFoldersSelect2.options.length - 1);
    }   
    while (baseFoldersSelectM.options.length > 1) { // Remet a zéro pour éviter les problèmes (select 3 & 3 pop up)
        baseFoldersSelectM.remove(baseFoldersSelectM.options.length - 1);
    }   
    while (keyFiles.options.length > 1) { // Remet a zéro pour éviter les problèmes (select 3 & 3 pop up)
        keyFiles.remove(keyFiles.options.length - 1);
    }   
    fetch('/basefolders') // On appelle la fonction correspondante à la route indiquée
        ...
        .then(baseFolders => {
            ...
            baseFolders.forEach(folder => { // Pour chaque dossier
                ...
                option4.value = folder; // La valeur de l'option sera le nom du dossier
                option4.textContent = folder; // La valeur affichée dans le select sera le nom du dossier
...
function loadMFiles(phone){
    ...
    while (Mf.options.length > 1) { // Remet a zéro pour éviter les problèmes 
    ...
    .then(response => response.json()) // On récupère la réponse
    .then(fil => {
        Mf.innerHTML = '<option value="">Choix fichier</option>';
...
function loadKFiles(phone){
    const Mf = document.getElementById('key2'); 
    while (Mf.options.length > 1) { // Remet a zéro pour éviter les problèmes 
        Mf.remove(Mf.options.length - 1);
    }
    fetch("/keyFiles", {
...
//--------------------------- Valeurs clés ---------------------------- bode (phase gain) facteur echelle, biais, alignement

function key_val(chemin)
{
    fetch('/kagi',{
        method: 'POST',
        headers: {
    ...
    .then(response => response.json()) // On récupère la réponse
    .then(result => {
        document.getElementById('tab_bode').style.display = "block";
        document.getElementById('bode_button').style.display = "block";
        document.getElementById('VS').style.display = "block";
        var bias = result.bias;
        var ladder = result.ladder;
        var matrix = result.matrix;
        displayTable(bias, document.getElementById('biAA'));
        displayTable(ladder, document.getElementById('laDD'));
        displayTable(matrix, document.getElementById('maTT'));
        var df = result.df;
        var dfcor = result.dfcor;
        var ctx = document.getElementById('chart_norm').getContext("2d");
        var ctx2 = document.getElementById('chart_filt').getContext("2d");
        displayChart_XYZ(df, "G", ctx, "ZER");
        displayChart_XYZ(dfcor, "G", ctx2, "ZAR");
        document.getElementById('data+').style.display = "block";
        document.getElementById('stdA').textContent = "Ecart type : " + Math.round(result.stdA * 1000) / 1000;
		document.getElementById('varA').textContent = "Variance : " + Math.round(result.varA * 1000) / 1000;
        document.getElementById('moyA').textContent = "Calcul/Obj (vit) : " + Math.round(result.moyA * 1000) / 1000 + " / " + result.vit + " rad/s";
        document.getElementById('stdB').textContent = "Ecart type : " + Math.round(result.stdB * 1000) / 1000;
		document.getElementById('varB').textContent = "Variance : " + Math.round(result.varB * 1000) / 1000;
        document.getElementById('moyB').textContent = "Calcul/Obj (vit) : " + Math.round(result.moyB * 1000) / 1000 + " / " + result.vit + " rad/s";
    })
    .catch(error => console.error('Erreur lors de la récupération du fichier', error)); // Si erreur
}

function openGraph(axis) {
    ...
    fetch('bodebo',{
        ...
        body: JSON.stringify({ key2:key2, axis:axis })
    })
    .then(response => response.json()) // On récupère la réponse
    .then(data => {
        plotBode(data)
        document.getElementById('bode-plot').style.display = 'block';
    })
    .catch(error => console.error('Erreur lors de la récupération du fichier', error)); // Si erreur
}

//---------------------------- Fonction affichage bode -----------------------

// Fonction pour tracer le diagramme de Bode avec Plotly
function plotBode(data) {
    const freqs = data.frequencies;
    const gain_db = data.gain_db;
    const phase_shift = data.phase_shift;
    ...
    Plotly.newPlot('bode-plot', dataPlot, layout);
}

//---------------------------- Affichage visible display -------------------------------
